...
resource "nomad_job" "prometheus" {
  ...
  jobspec = templatefile("../../jobs/prometheus.hcl", {
    ...
    config = {
      data        = file("../../config/${lookup(local.namespaces[var.deploy_environment][each.key], "prometheus_config", "minimal-prometheus")}/prometheus.yml.tpl")
      constraints = <<EOC
EOC
      tls = {
...
resource "nomad_job" "kibana" {
  jobspec = templatefile("../../jobs/kibana.hcl", {
    ...
    config = {
      ...
      elasticsearch_hosts = [
        "http://bo01-vm-es01.node.bo01.noroutine.me:9201",
        "http://bo01-vm-es02.node.bo01.noroutine.me:9201",
        "http://bo01-vm-es03.node.bo01.noroutine.me:9201",
      ]
      tls = {
        ca          = file(var.tls_ca_cert_file)
        client_cert = file(var.tls_client_cert_file)
