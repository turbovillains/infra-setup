...
curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sudo sh -s -- -b /usr/local/bin v0.48.1
...
(curl -sSL "https://get.helm.sh/helm-v3.13.3-linux-amd64.tar.gz" | sudo tar -C /usr/local/bin/ --no-same-owner --strip-components=1 -xzv linux-amd64/helm)

# skaffold
# https://github.com/GoogleContainerTools/skaffold/releases
sudo curl -sLo /usr/local/bin/skaffold https://storage.googleapis.com/skaffold/releases/v2.9.0/skaffold-linux-amd64 && sudo chmod +x /usr/local/bin/skaffold
...
sudo curl -sLo /usr/local/bin/ytt https://github.com/vmware-tanzu/carvel-ytt/releases/download/v0.46.3/ytt-linux-amd64 && sudo chmod +x /usr/local/bin/ytt
...
sudo curl -sLo /usr/local/bin/argocd https://github.com/argoproj/argo-cd/releases/download/v2.9.3/argocd-linux-amd64 && sudo chmod +x /usr/local/bin/argocd

# argo workflows
# https://github.com/argoproj/argo-workflows/releases
curl -sLo argocli.gz https://github.com/argoproj/argo-workflows/releases/download/v3.5.2/argo-linux-amd64.gz
gunzip argocli.gz
sudo mv argocli /usr/local/bin/argocli

# https://github.com/goreleaser/goreleaser/releases
curl -sLo- https://github.com/goreleaser/goreleaser/releases/download/v1.22.1/goreleaser-1.22.1-1-x86_64.pkg.tar.zst \
