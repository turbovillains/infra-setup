...
# kubectl
sudo curl -sLo /usr/local/bin/kubectl "https://dl.k8s.io/release/$(curl -sL https://dl.k8s.io/release/stable.txt)/bin/linux/${ARCHX}/kubectl" && sudo chmod +x /usr/local/bin/kubectl

# jq
# https://github.com/stedolan/jq/releases
# no jq so cannot use JQ_VERSION=$(curl -s "https://api.github.com/repos/jqlang/jq/releases/latest" | jq -r '.tag_name')
JQ_VERSION=jq-1.7.1
sudo curl -sLo /usr/local/bin/jq https://github.com/jqlang/jq/releases/download/${JQ_VERSION}/jq-linux-${ARCHX} && sudo chmod +x /usr/local/bin/jq

# cosign
# https://github.com/sigstore/cosign/releases
curl -sLO "https://github.com/sigstore/cosign/releases/latest/download/cosign-linux-${ARCHX}"
sudo mv cosign-linux-${ARCHX} /usr/local/bin/cosign
...
# crane
curl -sLo- "https://github.com/google/go-containerregistry/releases/download/$(curl -s "https://api.github.com/repos/google/go-containerregistry/releases/latest" | jq -r '.tag_name')/go-containerregistry_Linux_${ARCHY}.tar.gz" | sudo tar -C /usr/local/bin/ --no-same-owner -xzv crane krane gcrane
...
(curl -sSL "https://get.helm.sh/helm-${HELM_VERSION}-linux-${ARCHX}.tar.gz" | sudo tar -C /usr/local/bin/ --no-same-owner --strip-components=1 -xzv linux-${ARCHX}/helm)
...
sudo curl -sLo /usr/local/bin/skaffold https://storage.googleapis.com/skaffold/releases/${SKAFFOLD_VERSION}/skaffold-linux-${ARCHX} && sudo chmod +x /usr/local/bin/skaffold

# yq
# https://github.com/mikefarah/yq/releases
YQ_VERSION=$(curl -s "https://api.github.com/repos/mikefarah/yq/releases/latest" | jq -r '.tag_name')
sudo curl -sLo /usr/local/bin/yq https://github.com/mikefarah/yq/releases/download/${YQ_VERSION}/yq_linux_${ARCHX} && sudo chmod +x /usr/local/bin/yq
...
sudo curl -sLo /usr/local/bin/yj https://github.com/sclevine/yj/releases/download/${YJ_VERSION}/yj-linux-${ARCHX} && sudo chmod +x /usr/local/bin/yj
...
sudo curl -sLo /usr/local/bin/ytt https://github.com/carvel-dev/ytt/releases/download/${YTT_VERSION}/ytt-linux-${ARCHX} && sudo chmod +x /usr/local/bin/ytt
...
(curl -sSL "https://github.com/kubernetes-sigs/kustomize/releases/download/kustomize%2F${KUSTOMIZE_VERSION}/kustomize_${KUSTOMIZE_VERSION}_linux_${ARCHX}.tar.gz" | sudo tar -C /usr/local/bin/ --no-same-owner -xzv kustomize)

# argocd
# https://github.com/argoproj/argo-cd/releases
ARGOCD_VERSION=$(curl -s "https://api.github.com/repos/argoproj/argo-cd/releases/latest" | jq -r '.tag_name')
sudo curl -sLo /usr/local/bin/argocd https://github.com/argoproj/argo-cd/releases/download/${ARGOCD_VERSION}/argocd-linux-${ARCHX} && sudo chmod +x /usr/local/bin/argocd
...
curl -sLo /usr/local/bin/cfssl https://github.com/cloudflare/cfssl/releases/download/v${CFSSL_VERSION}/cfssl_${CFSSL_VERSION}_linux_${ARCHX} && sudo chmod +x /usr/local/bin/cfssl
